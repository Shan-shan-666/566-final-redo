...
total = total %>%
  mutate("case_fatality_rate(%)" = deaths_number/cases_number*100, "prevalence(%)" = cases_number/total_population*100, "death_rate" = deaths_number/total_population*1000) %>%
...
fig
```
These pie charts were built based on absolute number. We can see that for both case and death number, White contributed more than a half to the total number of them. Differently, for the case number, the proportion of LatinX is slightly larger than the Black, which is opposite on the death count. For both case and death, the White, Black and LatinoX contributed most to the total number,which make sense since those three races are the top 3 component of U.S.population. 
...
Now let's take a look at the case fatality rate.Interestingly, The top three high CFR are related with asian(4.7%), black(4.3%) and white(3.5%). This order is quite different with any previous ones. Take latiX for example, though it rank first in both prevalence and death rate, it only rank forth in the case fatality rate(3.2%). I think it could have something to do with people of different race's immune system. It might be a good research question for health care scholars.
